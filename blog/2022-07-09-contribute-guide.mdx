...
* GitLab 的話，在專案副本網頁上方會出現一則寫著 `You pushed to...` 的訊息，請點選下方的 `Create Merge Request` 按鈕：
...
這通常會發生在您第一次使用 Git Cli 進行 Git 專案管理的時候，因為他沒有您的資訊，所以 `git commit` 指令無法執行，請跟著他的指令提示填入您的相關資訊後，再輸入一次 `git commit` 即可正常提交。

![](/img/contribution-guide/NoGlobalAccountError.png)

#### 狀況二：`Nothing to commit, working tree clean`
...
這跟剛剛 `Nothing to commit, working tree clean` 差不多，您沒有提交需要被推送上 Git 平台。

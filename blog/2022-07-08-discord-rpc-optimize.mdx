...
![Vscode Discord Rich Presence Extension](/img/discord-rpc-customize/vscode-presence-extension.jpg)
...
![JetBrains Presence More Settings](/img/discord-rpc-customize/jetbrains-presence-more-settings.png)
...
![Npm install discord rpc](/img/discord-rpc-customize/npm-install-discord-rpc.png)
...
![Rider Name Solution](/img/discord-rpc-customize/name-solution.png)

建立完成以後，Rider會自動幫你開啟 `Program.cs`，此時請先別急，請先在視窗底部找到 `NuGet` 並點一下，此時會開啟 NuGet 模組管理，並在左上搜尋欄位中輸入 `DiscordRichPresence`，

![Nuget Install Discord RPC](/img/discord-rpc-customize/install-discord-rich-presence.png)

它會找到一個模組，點擊後右邊會顯示指令資訊，請點選 `nuget.org` 選單旁的 `+` 進行安裝

![Nuget Install Discord RPC](/img/discord-rpc-customize/install-discord-presence-package.png)

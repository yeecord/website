---
slug: fix-music-lag
title: Discord 音樂機器人一鍵解決聽音樂卡頓LAG問題
authors: kane
description: "你喜歡在 Discord 使用音樂機器人收聽你最喜歡的音樂，我猜可能是八軌錄音帶，但是你經常會遇到機器人卡頓，影響你的音樂體驗。在這篇講到的方法可以解決這個問題，並且可以順暢地聽音樂。"
tags: ["Discord", "dc", "音樂", "機器人", "2022", "Rythm", "卡頓", "LAG"]
image: /img/fix-music-lag/music.jpg
---
...
<head>
	<link rel="canonical" href="https://yeecord.com/blog/fix-music-lag/"/>
</head>

<TOCInline toc={toc}/>

![聽音樂](/img/fix-music-lag/music.jpg)

你喜歡在 Discord 使用音樂機器人收聽你最喜歡的音樂，~~我猜可能是八軌錄音帶~~，但是你經常會遇到機器人卡頓，影響你的音樂體驗。

### 為什麼機器人會卡頓？

常見 LAG 的狀況通常都是來自於延遲過高或是網路不穩的問題，透過切換 Discord 語音伺服器道不同區域來解決這類的問題。
...
![Windows網際網路設定](/img/fix-music-lag/windows-network-settings.jpg)
...
![Discord Status](/img/fix-music-lag/discord-status.png)

在這個頁面上會標示到目前是否有重大的服務離線，或是目前 API 有沒有被影響到，並且確認是否已經被修復。


### 3. 使用 Discord 用戶端 App
...
![Discord 語音頻道](/img/fix-music-lag/edit-channel-settings.png)

3. 在下方 `地區覆寫` 嘗試選擇不同的區域

![Discord 語音地區編輯](/img/fix-music-lag/edit-voice-region.png)
...
[YEE式機器龍](/docs/how-to-invite-bot/) 是繼 Rythm 停止營運之後最棒的中文音樂機器人選擇，對於剛開始使用 Discord 的新朋友們很友善，擁有全中文的介面，點這裡 [馬上開始使用](https://yeecord.com/invite/)

...
title: 如何設定自己的Discord豐富狀態（Rich Presence）
authors: [kay]
description: "此文章會教大家怎麼設定Discord豐富狀態（Rich Presence），讓你的朋友或伺服器成員更知道您現在在做什麼！"
tags:
  [
    "Discord",
    "dc",
...
# 如何設定自己的Discord豐富狀態（Rich Presence）

此文章會教大家設定Discord豐富狀態（Rich Presence）的多種方法，讓你的朋友或伺服器成員更知道您現在在做什麼！
<!--truncate-->

## 前言
Discord Rich Presence是一個可以顯示你目前狀態更多資訊的東西，我第一次知道是在我剛入坑Discord沒多久的時候，看到朋友都能顯示他在玩什麼遊戲、正在用什麼程式語言和IDE寫程式......後來才知道它是Discord Rich Presence。  
![](https://i.imgur.com/Am4pM7L.png)  
幾個月前，我還是Yeecord小幫手的時候，有一個成員在詢問說要怎麼搞自訂義狀態，雖然小幫手和管理員們有盡全力幫他（但我沒有，我當時不在），還有給他影片看，但是最後還是沒教成功，雙方不歡而散......  
所以今天就想......寫這篇文章吧！我也覺得我到目前對Yeecord真的貢獻還是太少了，現在也沒有什麼很critical的可以貢獻，就寫文章來貢獻Yeecord吧！或許也幫助那些Discord Rich Presence不會搞的人......
:::info 
...
當然認真的！有些軟體作者其實有把Discord Rich Presence程式碼寫進去他的軟體，你一開啟軟體就有了！或者沒有的話你進行一些設定就有了！  
比如Minecraft：
![](https://i.imgur.com/b2xjKiB.jpg)
## 方法二：使用插件
...
![](https://i.imgur.com/5Y28gtn.png)  
安裝完後，在Discord就能看到Rich Presence了  
![](https://i.imgur.com/NSBSg8T.png)  
狀態部分，Idling的話代表沒有在編輯檔案，有開啟檔案的話會顯示正在編輯的檔案，並且滑鼠移到大圖示上會顯示正在編輯什麼類型的檔案，移到大圖示右下角的小圖示也會顯示目前使用的軟體  
![](https://i.imgur.com/f0jXBzl.png)

### Intellij IDEA
Intellij IDEA的話是由JetBrains公司開發的一款整合式開發環境，社區版跟Visual Studio Code一樣有Git、Debug等功能，但只支援Java、Kotlin等少數程式語言。但升級至高級版後不但有內建社區版功能，還有內建DataBase Reader、遠端開發等超強功能，~~看完還不趕快砸錢砸起來～～~~  
回到插件，Intellij IDEA也有插件功能，並且也早就有人寫好Discord Rich Presence插件了，下方就教大家如何安裝並設定～～
#### 安裝插件
首先請按下鍵盤上的``Ctrl``+``Shift``+``S``開啟設定視窗，並點選左側``Plugins``前往插件管理頁面  
![](https://i.imgur.com/FyyeHHZ.png)

:::info NOTE
在沒有開啟專案的情況下，請在左側選單直接點選``Plugins``開啟插件管理頁面  
![](https://i.imgur.com/y8zFJA0.png)
:::

再來，請在上方搜尋``Discord Integration``，找到這個插件並按下插件名稱旁的``Install``按鈕進行安裝  
安裝成功後會顯示``Restart IDE``，點選該按鈕將IDE重新啟動。  
![](https://i.imgur.com/oQJA4dh.png)

#### 使用設定  
狀態顯示部分，請按下鍵盤上的``Ctrl``+``Shift``+``S``開啟設定視窗，並在左邊選單選擇``Discord``，即可看到Rich Presence顯示設定，通常保留預設值即可。
![](https://i.imgur.com/Y10HLMA.png)

:::info
這裡就先不詳細說每個設定欄位了，如果真的要設定的話可以自己先切換不同狀態的設定摸索一下，有些欄位等一下文章後面會介紹Discord官方的一個工具，也會讓您知道那些欄位是對應到什麼  
總共有三個狀態可以設定：
...
填好之後，再次按下連線就可以看到狀態成功顯示了！  
![](https://i.imgur.com/z0hWZuQ.png)

:::info CustomRP視窗下方的按鈕設定是什麼？
就是讓你的自訂義狀態能顯示按鈕，並且能設定按了以後能連至什麼往頁。``Text``是按鈕會顯示的文字，``URL``則是按下按鈕後會連至什麼網頁    
效果展示：  
![](https://i.imgur.com/dDufZtL.png)  
（注意：在電腦或網頁板Discord中，自己按按鈕按鈕是不會有事情發生的）
:::
...
* 如果您真的很想知道程式碼每個字是什麼意義，請自行上網Google了解物件導向、程式語言的相關知識！
...
<SyntaxHighlighter language="sh" showLineNumbers={true} style={dracula}>
  {`pip install pypresence`}
</SyntaxHighlighter>

安裝完成後，請在您覺得適合的地方建立一個Python檔案  
![](https://i.imgur.com/DR0XfxQ.png)
#### 開始撰寫程式碼
首先我們要先匯入PyPresence模組
<SyntaxHighlighter language="python" showLineNumbers={true} style={dracula}>
...
再來，創建Presence類別實體（用以連線並設定Rich Presence，）：
<SyntaxHighlighter language="python" showLineNumbers={true} style={dracula}>
...
然後，更新Rich Presence
<SyntaxHighlighter language="python" showLineNumbers={true} style={dracula}>
{`client.update(state="RPCTest")
print("更改狀態成功！")`}
</SyntaxHighlighter> 

:::info 解析
* 為了讓我們知道已經好了，所以加了一行印出字串讓我們知道好了
* 在此只示範更改state欄位，如果想更改更多欄位，請參考[PyPresence Docs](https://qwertyquerty.github.io/pypresence/html/info/quickstart.html)
:::
...
![](https://i.imgur.com/MX2rrAS.png)

建立完成以後，Rider會自動幫你開啟`Program.cs`，此時請先別急，請先在視窗底部找到``NuGet``並點一下，此時會開啟NuGet模組管理，此時請切換至Packages頁面，在左上搜尋欄位中輸入`DiscordRichPresence`，它會找到一個模組，點擊後右邊會顯示指令資訊，請點選``nuget.org``選單旁的``+``進行安裝  
![](https://i.imgur.com/jr1bVCn.png)
#### 開始撰寫程式碼
請先將裡面所有內容刪除，並匯入模組
<SyntaxHighlighter language="csharp" showLineNumbers={true} style={dracula}>
{`using DiscordRPC;

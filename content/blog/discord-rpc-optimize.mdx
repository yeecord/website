...
![Discord 自定義 遊戲狀態](/blog/discord-rpc-customize/discord-rpc-customize.jpeg)
...
![Minecraft Discord Rich Presence](/blog/discord-rpc-customize/minecraft-presence.jpg)
...
![Vscode Discord Rich Presence Extension](/blog/discord-rpc-customize/vscode-presence-extension.jpg)

安裝完後，在 Discord 就能看到遊戲狀態了，

<Adsense />

![Vscode Discord Rich Presence](/blog/discord-rpc-customize/vscode-presence.png)

狀態部分，Idling 的話代表沒有在編輯檔案，有開啟檔案的話會顯示正在編輯的檔案，並且滑鼠移到大圖示上會顯示正在編輯什麼類型的檔案，移到大圖示右下角的小圖示也會顯示目前使用的軟體。

![Vscode Discord Rich Presence Idling](/blog/discord-rpc-customize/vscode-presence-hover.png)
...
![JetBrains Settings](/blog/discord-rpc-customize/jetbrains-settings.png)

如果在沒有開啟專案的導覽頁面，請在左側選單直接點選 `Plugins` 開啟插件管理頁面。

![JetBrains Plugins Settings](/blog/discord-rpc-customize/jetbrains-plugins.png)

再來，請在上方搜尋 `Discord Integration`，找到這個插件並按下插件名稱旁的 `Install` 按鈕進行安裝
安裝成功後需要重新啟動 IDE ，點選 `Restart IDE` 將 IDE 重新啟動。

![JetBrains Install Plugin](/blog/discord-rpc-customize/jetbrains-install-plugin.png)
...
![JetBrains Discord Rich Presence Settings](/blog/discord-rpc-customize/jetbrains-presence-settings.png)
...
![JetBrains Presence More Settings](/blog/discord-rpc-customize/jetbrains-presence-more-settings.png)
...
![Create Discord Application](/blog/discord-rpc-customize/create-application.png)
...
<Callout type="warn" title="我想先警告你">
  Rich Presence Assets 上傳的圖片限制為：格式為 `.png`、`.jpg` 或
  `.jpeg`，建議圖片尺寸為 `1024 x 1024`，最小圖片尺寸為 `512 x 512`
</Callout>

![Discord Application Art Assets](/blog/discord-rpc-customize/application-art-assets.png)

### 下載軟體

請先前往 [CustomRP 軟體官網](https://www.customrp.xyz/) 點選 `Installer` 下載安裝版或 `Portable` 下載免安裝版（在此下載免安裝版）：

![CustomRP Homepage](/blog/discord-rpc-customize/custom-rp-download.png)

下載後，請將下載到的壓縮檔解壓縮。
解壓縮完成後請進入剛剛解壓出來的資料夾，點選 `CustomRP.exe` 開啟軟體，

![Open CustomRP](/blog/discord-rpc-customize/open-custom-rp.png)

若您剛剛下載安裝檔，請雙擊剛剛下載下來的安裝檔，依照安裝步驟進行安裝，然後在安裝完後開啟軟體。

![Install CustomRP](/blog/discord-rpc-customize/install-custom-rp.png)

第一次啟動時，會詢問您是否將使用情況分析提供給開發人員，這個將有助於軟體改善，可以自行決定要不要分享，

![CustomRP User Analytics](/blog/discord-rpc-customize/user-analysis.png)

之後就會看到軟體主介面了！

<Adsense />

![CustomRP](/blog/discord-rpc-customize/custom-rp.png)

### 填入資訊

請先前往剛才的應用程式設定，將 Application ID 複製，並貼入 CustomRP 的 ID 欄位：

![Copy Discord Application ID](/blog/discord-rpc-customize/copy-application-id.png)
![CustomRP Application ID](/blog/discord-rpc-customize/paste-id-to-custom-rp.png)

填完後，我們先按一下 ID 欄位旁的連線進行連線先確認 ID 是否正確，
如果 ID 欄位的底色變成綠色，恭喜您，它已經成功連線並將您的狀態設置成功了！

![CustomRP Application ID Success](/blog/discord-rpc-customize/paste-id-success.png)
...
![Discord Rich Presence Visualizer](/blog/discord-rpc-customize/presence-visualizer.png)
...
![CustomRP Example](/blog/discord-rpc-customize/custom-rp-example.png)

<Callout title="我想先告知你">
  CustomRP
  視窗下方的按鈕設定是什麼可以讓你的自訂義狀態能顯示按鈕，並且能設定按了以後能連至什麼網站。`Text`
  是按鈕會顯示的文字，`URL` 則是按下按鈕後會連至什麼網站。

![Discord Rich Presence Button](/blog/discord-rpc-customize/custom-button.png)
...
![Vscode create file](/blog/discord-rpc-customize/create-a-python-file.png)
...
![PyPresence](/blog/discord-rpc-customize/pypresence.png)
...
![Npm install discord rpc](/blog/discord-rpc-customize/npm-install-discord-rpc.png)
...
![Vscode Create File](/blog/discord-rpc-customize/create-index-js.png)
...
![Discord Rich Presence](/blog/discord-rpc-customize/discord-rpc-result.png)
...
![Rider New Solution](/blog/discord-rpc-customize/rider-new-solution.png)
...
![Rider Name Solution](/blog/discord-rpc-customize/name-solution.png)

建立完成以後，Rider 會自動幫你開啟 `Program.cs`，此時請先別急，請先在視窗底部找到 `NuGet` 並點一下，此時會開啟 NuGet 模組管理，並在左上搜尋欄位中輸入 `DiscordRichPresence`，

![Nuget Install Discord RPC](/blog/discord-rpc-customize/install-discord-rich-presence.png)

它會找到一個模組，點擊後右邊會顯示指令資訊，請點選 `nuget.org` 選單旁的 `+` 進行安裝

![Nuget Install Discord RPC](/blog/discord-rpc-customize/install-discord-presence-package.png)

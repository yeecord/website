...
![如何貢獻 Git 平台上的專案](/blog/git-contribution-guide/git-contribution-guide.jpeg)
...
![Fork 按鈕](/blog/git-contribution-guide/github-where-fork-button.png)

`Repository name` 預設會是該專案的名字，然後點選 `Create Fork` 創建：

![寫入 Fork 資訊](/blog/git-contribution-guide/github-write-fork-information.png)

<Adsense />

GitLab 的 Fork 部分其實跟 GitHub 大同小異，多了能決定專案是否此副本能被別人看到的 `Visibility Level` 選項。

<Callout title="小提示">`Project Url` 欄位後面設定的是這個副本的擁有者是誰</Callout>

![寫入 Fork 資訊](/blog/git-contribution-guide/gitlab-write-fork-information.png)

複製完成後，他會幫你跳轉至您的副本專案網頁，至此您就完成了 Fork 專案，並且您會在專案標題下方看到 `Fork from foo/bar`。

![成功 Fork](/blog/git-contribution-guide/github-fork-finished.png)

![成功 Fork](/blog/git-contribution-guide/gitlab-fork-finished.png)

## 步驟二：Clone 專案副本、環境設定與開始編輯

首先我們到 GitHub 複製專案連結，點選 `Code` 按鈕 `HTTPS` 來複製連結，

![Code 按鈕](/blog/git-contribution-guide/github-where-code-button.png)c
...
![Clone 專案](/blog/git-contribution-guide/clone-project.png)
...
![新增 Upstream](/blog/git-contribution-guide/add-upstream.png)
...
![切換至新分支](/blog/git-contribution-guide/change-to-new-branch.png)

然後就可以開始編輯了！

![開始編輯](/blog/git-contribution-guide/start-contribution.png)
...
![追蹤並提交](/blog/git-contribution-guide/add-and-commit.png)

## 步驟三：向原專案發出推送要求

請先前往 Git 平台上專案副本網頁，點選 `Contribute` -> `Open Pull Request`

![開啟 Pull Request](/blog/git-contribution-guide/github-where-open-pr-button.png)

<Callout title="小筆記">

- GitHub 到專案頁面，如果有出現 `XXX had recent pushes...` 也可以按下該黃色橫幅右側的 `Compare & pull request`：

![Compare & pull request](/blog/git-contribution-guide/compare-and-pr-message.png)

- GitLab 到專案頁面，在專案副本網頁上方會出現一則寫著 `You pushed to...` 的訊息，請點選下方的 `Create Merge Request` 按鈕：

![Create Merge Request](/blog/git-contribution-guide/gitlab-where-open-pr-button.png)

</Callout>

接下來就會出現填寫 Pull Request 資訊的介面，填好相關資訊以後請按下 `Create Pull Request` 按鈕正式向原專案發出推送要求。

![寫入 Pull Request 資訊](/blog/git-contribution-guide/write-pr-information.png)

創建完成之後將會透過 Email 通知該 Pull Request 的各種更新動態，
![等待合併](/blog/git-contribution-guide/wait-for-merge.png)
...
![Pull Request 通知](/blog/git-contribution-guide/pr-notification.png)
...
![成功合併](/blog/git-contribution-guide/pr-merged.png)
...
<Callout type="warn" title="我想先告知你">
...
![Please tell me who you are](/blog/git-contribution-guide/no-global-account-error.png)

#### `Nothing to commit, working tree clean`

這代表您沒有對檔案做出變更，或是沒有使用 `git add` 指令先將檔案加入到 Git 索引內。

![Nothing to commit, working tree clean](/blog/git-contribution-guide/commit-working-tree-clean.png)
...
![Everything up-to-date](/blog/git-contribution-guide/push-up-to-date.png)
...
![Support for password authentication was removed...](/blog/git-contribution-guide/password-auth-error.png)
